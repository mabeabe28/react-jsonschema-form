...
function validateFormData(formData, schema, customValidate, transformErrors) {
  ...
  var filteredData = filterEmptyValues(formData, schema);

  var _jsonValidate = (0, _jsonschema.validate)(filteredData, schema),
  ...
  var errorHandler = customValidate(filteredData, createErrorHandler(filteredData));
...
function filterEmptyValues(data, schema) {
  var filtered = {};
  for (var prop in data) {
    if (data.hasOwnProperty(prop)) {
      // recurse if object
      if (_typeof(data[prop]) === "object" && !(data[prop] instanceof Array) && schema && schema["properties"]) {
        filtered[prop] = filterEmptyValues(data[prop], schema["properties"][prop]);
        // otherwise just check for empty strings
      } else {
        if (data[prop] !== "" || schema["required"] && schema["required"].includes(prop)) {
